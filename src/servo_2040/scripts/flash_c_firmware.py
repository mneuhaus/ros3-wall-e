#!/usr/bin/env python3
"""
Flash C firmware to Servo 2040 board using picotool.
"""
...
def flash_firmware(firmware_path):
    """Flash C firmware using picotool."""
    if not os.path.exists(firmware_path):
        print(f"Error: Firmware not found at {firmware_path}")
        print("Did you build the C firmware first? Try:")
        print("cd src/servo_2040/firmware/c_version && mkdir build && cd build && cmake .. && make")
        return False
    ...
    try:
        if wait_for_bootsel():
            if flash_firmware(firmware_path):
                print("\nC firmware flashed successfully!")
            else:
                sys.exit(1)
